...
export const getStaticPaths = async () => {
  const res = await fetch(`https://api.pandascore.co/matches/running?sort=&page=1&per_page=50&token=${process.env.TOKEN}`);
...
export const getStaticProps = async (context) => {
  ...
  const res = await fetch(`https://api.pandascore.co/matches/running?search[slug]=${slug}&token=${process.env.TOKEN}`);
